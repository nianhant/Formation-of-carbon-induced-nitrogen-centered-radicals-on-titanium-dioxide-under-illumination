...
def ads_slab_eng(path):
    ...
    for root, dirs, files in os.walk(cur_dir, topdown=True):
        ...
        if 'ensemble_np.txt' in files:
            ...
            adsorbate = adsorbate.split('_')[0]
            eng_dict[adsorbate] = np.array(result)*13.605698 + DFT_eng
    return eng_dict

Thiel_dict = ads_slab_eng('../ads_slab')
slab_dict = ads_slab_eng('../slab')
# print(slab_dict)
...
Thiel_ensemble = get_potential_energy(ref_dict, Thiel_dict, slab_dict)

# print(Thiel_ensemble['N2'].mean())
# print(Thiel_ensemble['N2'].std())

Thiel_ensemble['H'] = np.array([0])

ensemble_dict = dict()
for en_dict in [Thiel_ensemble]:
